...
class TestUtils(pwtests.BaseTest):
    ...
    def _createSet(self, size=10):
        ...
        print("id index       file sr")
        for i in range(10):
            img = MockImage()
            img.setLocation(i + 3, stackFn)
            img.setSamplingRate(i % 3)
            imgSet.append(img)
            if i == 4:
                sleep(1)
            ...
            # Print the item
            print("{:2d} {:5d} {:5s} {:2d}".format(img.getObjId(), img.getIndex(), stackFn, i % 3))
        imgSet.write()

        # Test size is 10
        self.assertSetSize(imgSet, 10)

        return imgSet, halfTimeStamp

    def test_Semminar(self):

        imgSet, halfTimeStamp = self._createSet()

        # # Test hasChangedSince
        # timeStamp = dt.datetime.now()
        # self.assertFalse(imgSet.hasChangedSince(timeStamp), "Set.hasChangedSince returns true when it hasn't changed.")
        # # Remove 10 seconds
        # self.assertTrue(imgSet.hasChangedSince(timeStamp - dt.timedelta(0, 10)),
        #                 "Set.hasChangedSince returns false when it has changed.")
        ...
        # Looping a set
        print("Looping a set the traditional way...")
        for item in imgSet.iterItems():
            print(item)
